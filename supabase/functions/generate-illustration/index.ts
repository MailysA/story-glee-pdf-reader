...
serve(async (req) => {
  ...
  try {
    ...
    let prompt;
    if (pageContent) {
      // Générer une illustration basée sur le contenu spécifique de la page
      prompt = `Une illustration colorée et douce pour enfants représentant cette scène: "${pageContent}". Style livre d'enfants, couleurs vives et rassurantes, adapté aux enfants`;
    } else {
      // Fallback sur les prompts thématiques existants
      ...
      prompt = illustrationPrompts[theme] || `Une belle illustration pour enfants montrant ${childName} dans une aventure ${theme}, style livre d'enfants coloré et rassurant`;
    }
